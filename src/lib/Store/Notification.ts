...
import type { InsertNotification, UpdateNotification } from '$lib/Supabase/Types/database.types';
...
const createNotificationStore = () => {
    const { subscribe, update, set } = writable<Store<NotificationEntity>>(new Store<NotificationEntity>());
    ...
    return {
        ...
        put: async (request: UpdateNotification) => {
            try {
                ...
                update((notifications) => {
                    const index = notifications.data.findIndex((notification) => notification.id === request.id);
