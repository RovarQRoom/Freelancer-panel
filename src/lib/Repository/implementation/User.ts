...
export class UserRepository implements IUsersRepository {
	async createUserAsync(request: InsertUser): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').insert(request).select("*").returns<UserEntity>().single();
		if (response.error) {
	...
	async readUserAsync(id: number): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').select("*").eq('id', id).returns<UserEntity>().single();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
	...
	async readUsersAsync(options?: GenericListOptions): Promise<PostgrestSingleResponse<Array<UserEntity>>> {
        const query = supabase
        .from("User")
        .select(options?.select ?? "*", { count: "exact" });
      if (options?.search)
        query.textSearch(options.fieldOption ?? "name", options.search);
	...
	async readUserByEmailAsync(email: string): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').select("*").eq('email', email).returns<UserEntity>().single();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
	...
	async readUserByPhoneAsync(phone: string): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').select("*").eq('phone', phone).returns<UserEntity>().single();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
	...
	async updateUserAsync(request: UpdateUser): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').update(request).eq('id', request.id!).select("*").returns<UserEntity>().single();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
	...
	async deleteUserAsync(id: number): Promise<PostgrestSingleResponse<UserEntity>> {
		const response = await supabase.from('User').update({ deleted_at: new Date().toISOString() }).eq('id', id).select("*").returns<UserEntity>().single();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
