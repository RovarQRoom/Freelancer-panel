...
export class NotificationRepository implements INotification {
	async createNotificationAsync(
		request: InsertNotification
	): Promise<PostgrestSingleResponse<NotificationEntity>> {
		const response = await supabase
			.from('Notification')
			.insert(request)
			.select('*,users:NotificationUser(user(id),seen)')
	...
	async createNotificationUserAsync(
		request: InsertNotificationUser[]
	): Promise<PostgrestSingleResponse<NotificationUserEntity[]>> {
		const response = await supabase
			.from('NotificationUser')
			.insert(request)
			.select('*,user(id,name),notification(id)')
			.returns<NotificationUserEntity[]>();
		if (response.error) {
			toastStore.error(response.error.message);
			throw new Error(response.error.message);
		}
	...
	async readNotificationsAsync(
		options?: GenericListOptions
	): Promise<PostgrestSingleResponse<NotificationEntity[]>> {
		const query = supabase
			.from('Notification')
			.select(options?.select ?? '*, users:NotificationUser(user:User(id,name))', {
				count: 'exact'
			});
