<script lang="ts">
    ...
    async function handleUpsert() {
        ...
        try {
            titleResponse = upsertSubcategoryTitle.id ? await languageStore.put(upsertSubcategoryTitle) : await languageStore.insert(upsertSubcategoryTitle);
            descriptionResponse = upsertSubcategoryDescription.id ? await languageStore.put(upsertSubcategoryDescription) : await languageStore.insert(upsertSubcategoryDescription);
            showModal = false;
            upsertSubcategory.title = titleResponse?.id ?? 0;   
            upsertSubcategory.description = descriptionResponse?.id ?? 0;
            subcategoryResponse = upsertSubcategory.id ? await subcategoryStore.put(upsertSubcategory) : await subcategoryStore.insert(upsertSubcategory);
        } catch (error) {
            if(titleResponse) {
                    ...
                    {#each Object.keys(Languages) as language}
                        <TabItem open={language === Languages.EN} title={language}>
                            ...
                            placeholder={m.description()}
                                bind:value={upsertSubcategoryDescription[language.toLowerCase() as keyof InsertLanguage]}
                                required={language === Languages.EN}
                                rows={4}
                            />
                        </TabItem>
