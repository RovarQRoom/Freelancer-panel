<script lang="ts">
    ...
    onMount(async () => {
        roles = await roleStore.fetchForDropdown({
            select: 'id,name',
            limit: 100
        }) ?? [];
    });
    ...
    async function handleAddUser() {
        ...
        try {
            ...
            // Create user
            const response = await userStore.insert(createUser);
    ...
    >
        ...
        <div class="space-y-2">
            ...
            <div class="flex justify-center">
                <div class="relative h-64 w-64 overflow-hidden rounded-full bg-gray-100 transition-all duration-300 hover:shadow-lg">
                    {#if imageFile.preview}
                        ...
                        <button
                            type="button"
                            class="absolute bottom-4 right-4 p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-all duration-300"
                            onclick={() => {
                                if (imageFile.preview) {
